...
from sim.tes_models.events import OrderAcceptedData, OrderAccepted
from sim.tes_persistence.runs import save_run


def test_replay_executes(tmp_path, capsys) -> None:
    event = OrderAccepted(type="OrderAccepted", data=OrderAcceptedData(order_id=1, side="BUY", price=100, qty=2))
    save_run(
        base_dir=tmp_path,
        run_id="run-001",
        events=[event],
